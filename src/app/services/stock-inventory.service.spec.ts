...
fdescribe('StockInventoryService', () => {
  ...
  const mockProduct: Product = { "id": "1", "price": "16.00", "name": "Goody" };

  const url = 'https://5f51071d5e98480016123523.mockapi.io';

  const stockService = new StockInventoryService(null);
  ...
  it('should fetch single product', () => {
    service.getProduct(1).subscribe(product => {
      ...
      expect(mockProduct.name).toBe(product.name);
      expect(mockProduct.id).toBe(product.id);
      expect(mockProduct.price).toBe(product.price);
    });
  ...
  it('should return the products', () => {
    const spy = spyOn(stockService, 'getProducts').and.returnValue(of(mockProductList));
    expect(spy).toHaveBeenCalled();
  });

  it('should return a single product', () => {
    spyOn(stockService, 'getProduct').and.returnValue(of(mockProduct));
  });
